...
LOGIC_RUBY_RVM_VERSION=$(prompt_logic_colorize $LOGIC_COLOR_RUBY_RVM_VERSION $LOGIC_SYMBOL_RUBY_RVM_VERSION) 

prompt_logic_get_full_ruby_version() {
  (( $+commands[ruby] )) && LOGIC_RUBY_VERSION_FULL="$(ruby -v)"
}

prompt_logic_ruby_version() {
  [[ $LOGIC_RUBY_VERSION_FULL =~ 'ruby ([0-9a-zA-Z.]+)' ]]
  LOGIC_RUBY_VERSION=$match[1]
}

prompt_logic_get_full_rvm_version() {
  (( $+commands[rvm] )) && LOGIC_RVM_VERSION_FULL="$(rvm -v)"
}

prompt_logic_rvm_version() {
  [[ $LOGIC_RVM_VERSION_FULL =~ 'rvm ([0-9a-zA-Z.]+)'  ]]
  LOGIC_RVM_VERSION=$match[1]
}

logic_prompt_ruby_setup() {
  (( $+commands[ruby] )) || return 1
}

logic_prompt_ruby_check() {}

prompt_logic_current_rvm_gemset_name() {
...
logic_prompt_ruby_render() {
  ...
  if (( $+commands[rvm] )); then
      ...
      local rvm_gemset_name=$( prompt_logic_current_rvm_gemset_name )
